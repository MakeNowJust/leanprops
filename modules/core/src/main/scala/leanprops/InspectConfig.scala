...
/** `InspectConfig` haves a configuration for `inspect` method.
  ...
  */
case class InspectConfig(size: Int, singleLine: Boolean, inner: () => InspectConfig)
...
object WithInspectConfig {
  ...
  def inspect1[A: Inspectable](a: A)(f: String => String): WithInspectConfig[String] = inner(inspect(a).map(f))

  def inspect2[A: Inspectable, B: Inspectable](a: A, b: B)(f: (String, String) => String): WithInspectConfig[String] =
    inner(for { s1 <- inspect(a); s2 <- inspect(b) } yield f(s1, s2))

  def inspect3[A: Inspectable, B: Inspectable, C: Inspectable](a: A, b: B, c: C)(
      f: (String, String, String) => String): WithInspectConfig[String] =
    inner(for { s1 <- inspect(a); s2 <- inspect(b); s3 <- inspect(c) } yield f(s1, s2, s3))

  def inspect4[A: Inspectable, B: Inspectable, C: Inspectable, D: Inspectable](a: A, b: B, c: C, d: D)(
      f: (String, String, String, String) => String): WithInspectConfig[String] =
    inner(for {
      s1 <- inspect(a); s2 <- inspect(b); s3 <- inspect(c); s4 <- inspect(d)
    } yield f(s1, s2, s3, s4))

  def inspect5[A: Inspectable, B: Inspectable, C: Inspectable, D: Inspectable, E: Inspectable](
      a: A,
      b: B,
      c: C,
      d: D,
      e: E)(f: (String, String, String, String, String) => String): WithInspectConfig[String] =
    inner(for {
      s1 <- inspect(a); s2 <- inspect(b); s3 <- inspect(c); s4 <- inspect(d);
      s5 <- inspect(e)
  ...
  def sequenceBinding[A](b: Binding[WithInspectConfig[A]]): WithInspectConfig[Binding[A]] =
    b match {
      case Binding(vs, r) =>
        WithInspectConfig(c => Binding(vs.map(_.map(_.run(c))), r.map(_.run(c))))
    }

  def sequenceBindings[A](bs: Seq[Binding[WithInspectConfig[A]]]): WithInspectConfig[Seq[Binding[A]]] =
    sequence(bs.map(sequenceBinding))

  def sequenceResult[A](r: Result[WithInspectConfig[A]]): WithInspectConfig[Result[A]] = r match {
    case Result(vs, ok) =>
      WithInspectConfig(c => Result(vs.map(_.map(_.run(c))), ok))
  }

  def sequenceResults[A](rs: Seq[Result[WithInspectConfig[A]]]): WithInspectConfig[Seq[Result[A]]] =
    sequence(rs.map(sequenceResult))
  ...
  def cons2[A: Inspectable, B: Inspectable](name: String, a: A, b: B): WithInspectConfig[String] =
    inspect2(a, b)((s1, s2) => s"$name($s1, $s2)")

  def cons3[A: Inspectable, B: Inspectable, C: Inspectable](name: String, a: A, b: B, c: C): WithInspectConfig[String] =
    inspect3(a, b, c)((s1, s2, s3) => s"$name($s1, $s2, $s3)")

  def cons4[A: Inspectable, B: Inspectable, C: Inspectable, D: Inspectable](name: String,
                                                                            a: A,
                                                                            b: B,
                                                                            c: C,
  ...
  def cons5[A: Inspectable, B: Inspectable, C: Inspectable, D: Inspectable, E: Inspectable](
      name: String,
      a: A,
      b: B,
      c: C,
    ...
    inspect5(a, b, c, d, e)((s1, s2, s3, s4, s5) => s"$name($s1, $s2, $s3, $s4, $s5)")
}
